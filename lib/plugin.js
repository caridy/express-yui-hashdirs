...
function bundleUpdated(evt, api) {
    var yuibuild = evt.bundle.yuiBuildDirectory,    // only set by express-yui
    ...
    return key && meta && api.promise(function rename(resolve, reject) {
        ...
        function afterHash(err, results) {
            ...
            if (err) {
                ...
                // express-yui needs this for loader meta
                json = JSON.stringify(results, null, 2);

                // this triggers another call to all plugins with one evt.file
                api.writeFileInBundle(evt.bundle.name, file, json).then(resolve);
            }
        }
